...
public class RefreshToken {
    ...
    public void configureTasks() {
        ...
        String accessToken = Gcache.config.getAccessToken();
        String uid = Gcache.config.getUid();
        String tokenPwd = Gcache.config.getTokenPwd();
        ...
        if (StringUtils.hasText(accessToken) && StringUtils.hasText(uid) && StringUtils.hasText(tokenPwd)) {
            ...
            MasRsp<NewToken> refreshToken = JSON.parseObject(resp2, new TypeReference<>() {
            });
            checkResp(refreshToken);
